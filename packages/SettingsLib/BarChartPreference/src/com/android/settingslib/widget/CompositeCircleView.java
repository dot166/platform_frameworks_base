 ...
 * {@link #configure(float, int[], int[], int, TextView[])}.
 */
public class CompositeCircleView extends FrameLayout {
    ...
    private Pair<Integer, String>[] mValues;

    /**
     * Angles toward the middle of each colored partial circle, calculated in
     * {@link #configure(float, int[], int[], int, TextView[])}. Can be used to position text relative to the
     ...
     */
    public void configure(float startAngle, Pair<Integer, String>[] values, int[] colors, int strokeWidth,
                          TextView[] labels) {
        removeAllViews();
        mValues = values;
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            float sweepAngle = (values[i].first / total) * allocatedDegrees;
            pcv.setSweepAngle(sweepAngle);

            if (labels[i] != null) {
                int percentage = Math.round((values[i].first / total) * 100);
                String contextDescription = getContext().getString(
                        R.string.circle_view_usage_percent, labels[i].getText(), percentage);
